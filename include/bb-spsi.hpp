		...
		public:
		   ...
		   return root->contains_r(x);
			...
			void insert(uint64_t i, uint64_t x) {
				message_buffer.push_back(insert_message(i, x));
			...
			void remove(uint64_t i) {
				message_buffer.push_back(remove_message(i));
				if (message_buffer.size() == message_buffer.capacity()) {
					flush_messages();
					return;
				}
			...
			uint64_t at(uint64_t i) const {
				...
				return root->at(i, &message_buffer);
			}
			...
			void flush_messages() {
				for (message& message : message_buffer) {
					if (message.type == message_type::insert) {
						assert(message.index <= root->size());

						node<leaf_type, B, B_LEAF>* new_root = root->insert(message.index, message.value);
						...
						node<leaf_type, B, B_LEAF>* new_root = root->remove(message.index);
