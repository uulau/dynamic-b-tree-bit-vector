...
namespace dyn {
	...
	public:
		...
		void insert_proper() {
			...
			auto first_falling_out = 0;
			auto second_falling_out = 0;

			// Apply first insertion, store number falling out
			if (first_word_start_index < buffer_index) {
				first_falling_out = (words[first_word] >> 63) & uint64_t(1);
			...
			if (second_word_start_index < buffer2_index) {
				second_falling_out = (words[second_word] >> 63) & uint64_t(1);
				uint64_t word = words[second_word];
				uint64_t one_mask = (uint64_t(1) << second_index) - 1;
				uint64_t zero_mask = ~one_mask;
				uint64_t unchanged = word & one_mask;
				word <<= 1;
